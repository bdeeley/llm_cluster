...
def find_ip_prioritised(
    ...
    ip_to_name = {iface.ip_address: iface.name for iface in other_network.interfaces}

    def _is_valid_candidate_ip(ip: str) -> bool:
        # Ring hostfile entries are consumed as ip:port and currently require
        # routable IPv4 addresses for reliable parsing and dialing.
        if not ip:
            return False
    ...
    ips = [ip for ip in ips if _is_valid_candidate_ip(ip)]

    if ring:
        # Avoid container bridge/local virtual interfaces for ring links when
        # a routable alternative exists.
        routable_ips = [
            ip
            for ip in ips
            if not ip_to_name.get(ip, "").startswith(("docker", "br-", "veth", "lo"))
        ]
        if routable_ips:
            ips = routable_ips

    if not ips:
        # Fallback for asymmetric topology visibility: use peer interface IPs.
        fallback_ips = [
            iface.ip_address
            for iface in other_network.interfaces
            if _is_valid_candidate_ip(iface.ip_address)
        ]
        if not fallback_ips:
            return None
        ips = fallback_ips
...
def get_mlx_ring_hosts_by_node(
    ...
    - Self position: Host(ip="0.0.0.0", port=<that node's ring port>)
    - Left/right neighbors: actual connection IPs with the peer node's ring port
    - Non-neighbors: Host(ip="198.51.100.1", port=0) placeholder (RFC 5737 TEST-NET-2)

    A single shared ring port breaks when multiple logical nodes live on the same
    machine, because remote peers cannot distinguish two ranks behind the same IP.
    We therefore allocate one port per node and embed that directly in hosts_by_node.
    """
    ...
    ports_by_node = {
        node_id: (ephemeral_port if world_size == 1 else random_ephemeral_port())
        ...
        for idx, other_node_id in enumerate(selected_cycle):
            if idx == rank:
                hosts_for_node.append(
                    Host(ip="0.0.0.0", port=ports_by_node[other_node_id])
                )
                continue
            ...
            hosts_for_node.append(
                Host(ip=connection_ip, port=ports_by_node[other_node_id])
            )

        # MLX ring expects the local bind endpoint as the first host entry for
        # each rank. Keep self first to avoid rank>0 binding peer ports.
        hosts_for_node = hosts_for_node[rank:] + hosts_for_node[:rank]
